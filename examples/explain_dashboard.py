...
reader sees per case — and lets them interrogate it live. Two audiences, one
toggle: the page opens in a plain-language mode (no jargon, percentages
instead of logits, "handed to a person" instead of "declined"), and an
"Advanced" switch reveals the full technical register (margins, standardized
units, τ*, the exact numbers). Both variants of every sentence are rendered
into the file; the toggle flips a single CSS class, so switching is instant
and never interrupts a slider drag. Features:
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  kbd { background:var(--chip); border-radius:4px; padding:0 5px;
        font-size:11px; }
  /* two-audience text: .simponly shows in plain-language mode, .advonly in
     advanced mode; the toggle flips one body class, nothing re-renders */
  body.simple .advonly { display:none !important; }
  body.adv .simponly { display:none !important; }
  .modebar { float:right; margin-top:2px; }
</style>
</head>
<body class="simple">
<header>
  <div class="modebar">
    ...
    <span class="chip" id="modeAdv" title="full technical detail: margins, logits, standardized units">Advanced</span>
  </div>
...
<main>
  <div class="card caveat">
    ...
    <span class="advonly">
      ...
      flip buttons, the threshold explorer — asks a <b>score-space</b> question
      of the gate: <i>what would the model's inputs need to be</i> for the case
      to clear the answering bar. These are <b>not</b> causal claims, not
      treatment suggestions, and not clinically achievable actions (features
      are not independently changeable). A case flipped by the minimal change
      is answered at the <b>weakest possible confidence</b>, a hair above the
      bar itself. The error-rate certificate is a
  ...
  <div class="card">
    ...
    <div class="note">
      ...
      <span class="advonly">Cohort confidence-score distribution. Red bars sit
        ...
        τ* = <span id="taulab"></span>.</span></div>
  </div>
...
<footer>
  ...
  <span class="advonly">Generated by <code>examples/explain_dashboard.py</code>.
    The page recomputes the head's own arithmetic (float64) locally: sliders
    and flip buttons re-run the deployed answering rule, so what you see is the
    ...
    come from <code>certgate.explain.counterfactual_to_answer</code>
    (SPEC "explain.py"). Self-contained file, no network access.</span>
</footer>
<script>
"use strict";
...
// ---- mode toggle: one body-class flip, nothing re-renders ----
function setMode(m) {
...
function renderDetail() {
  ...
  h += "<h2 style='margin-top:10px'>" +
       ...
       dual("Every slider is live. Moving one only asks the program a " +
            ...
            "Every slider is live. Moving one asks the gate a question; it " +
            "does not model the patient. A <span style='color:var(--dec)'>" +
            "&#8634;</span> appears on any changed input — click it to " +
            "restore that input's recorded value.") + "</p>";
  xCur.forEach((v,j) => {
  ...
  if (c.declined && c.counterfactuals.length) {
    h += "<h2 style='margin-top:12px'>" +
      ...
      dual("Each row is the smallest change to ONE value that would let the " +
           "program answer — and even then it would only just clear the bar " +
           "(the least-confident answer it is allowed to give).",
...
// scope to the sidebar: the header's mode chips are .chip too and must keep
// their own handlers (an unscoped selector silently overwrote them)
document.querySelectorAll(".side .chip").forEach(ch => ch.onclick = () => {
  document.querySelectorAll(".side .chip").forEach(c => c.classList.remove("on"));
  ch.classList.add("on");
  filter = ch.dataset.f;
  rebuildList();
});
