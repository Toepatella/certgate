...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .gaxis { display:flex; justify-content:space-between; font-size:11px;
           color:var(--mut); }
  .srow { display:grid; grid-template-columns:104px 1fr 74px; gap:8px;
...
function gauge(score) {
  ...
  return "<div class='gauge'>" +
    "<div class='tau' data-l='bar' style='left:" + ((TAU-0.5)/0.5*100).toFixed(2) + "%'></div>" +
    "<div class='needle' style='left:calc(" + left.toFixed(2) + "% - 4px)'></div>" +
    "</div><div class='gaxis'><span>0.50</span><span>score " +
    score.toFixed(4) + "</span><span>1.00</span></div>";
}
...
function renderDetail() {
  ...
  const lead = lg >= 0 ? 1 : -1;
  let h = "<p><span class='verdict " + (answered ? "answered" : "declined") +
    "'>" + (answered ? "ANSWERED" : "DECLINED") + "</span>" +
    (modified ? "<span class='badge'>inputs modified — live what-if</span>" : "") +
    " · model leans <b>" + (lg >= 0 ? "positive" : "negative") + "</b>" +
    " · risk score " + p.toFixed(4) +
    (answered ? "" : " · needs " +
      (Math.log(TAU/(1-TAU)) - Math.abs(lg)).toFixed(4) +
      " more logit-confidence") + "</p>";
  h += gauge(score);
  ...
  h += "<button id='btnReset'>Reset case</button></p>";
  h += "<h2 style='margin-top:10px'>What-if: move an input, the gate re-decides" +
       "</h2><p class='note'>Moving a slider asks the gate a question; it does " +
       "not model the patient.</p>";
  xCur.forEach((v,j) => {
    ...
    h += "<div class='srow'><label title='" + NAMES[j] + "'>" + NAMES[j] +
      ...
      "'><span class='v num' id='sv" + j + "'>" + (+v).toFixed(3) + "</span></div>";
  });
  h += "<h2 style='margin-top:12px'>What drives the confidence</h2>" +
    ...
    "<div id='phibars'>" + bars(phiOf(xCur), lead) + "</div>";
  ...
  box.querySelectorAll("input[type=range]").forEach(sl => {
    sl.addEventListener("input", () => {
      ...
      xCur[j] = +sl.value;
      document.getElementById("sv"+j).textContent = (+sl.value).toFixed(3);
      liveUpdate(c);
    });
  });
...
function liveUpdate(c) {
  // re-render only verdict/gauge/bars for slider smoothness
  renderDetail.pending = renderDetail.pending || requestAnimationFrame(() => {
    renderDetail.pending = null;
    const keep = [];
    document.querySelectorAll("#detail input[type=range]").forEach(sl =>
      keep.push(+sl.value));
    renderDetail();
    // restore focus-less slider positions (values already in xCur)
  });
}
