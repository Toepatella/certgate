...
**RESOLVED 2026-07-30: the pre-extract freeze commit exists.** Operator-checklist step 0 was run before any extract was downloaded: `git init` + full-tree commit **`9f25b491b2554d0a4bd7aaaf44081c185d01715f`** ("freeze: eICU-CRD v2.0 protocol + constant pins, pre-extract", 2026-07-30 22:59 EDT), pushed to `github.com:Toepatella/certgate` on `main`. That hash is the pre-registration reference the paper cites for §0's ordering claim. Staging was verified corpus-clean at commit time (the only eICU-named tracked files are the five source/doc files; zero `*.csv.gz`). The eICU extract itself must still **never** be committed — `.gitignore` is now live and denies it by default.
