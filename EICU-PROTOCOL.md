...
Every A1–A5 change is a **tightening**: each adds a measurement or a refusal, and none relaxes
...
| T-2 | `-1` flows through as a finite value | `sentinels[t][col]`: `n_minus_one`, `n_other_negative`, `min_positive`, `p01` | negative mass not exactly at `-1.0` raises `unexpected-negative-sentinel` |
